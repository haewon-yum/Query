...
-- Replace 'id1112407590' with the app bundle ID (e.g. 'id1112407590')
-- Replace 'zp_npuall' with the event name (e.g. 'zp_npuall')
...
WHERE DATE(timestamp) >= DATE_SUB(CURRENT_DATE(), INTERVAL 14 DAY)
  AND app.bundle = 'id1112407590'
  AND LOWER(event.name) = LOWER('zp_npuall')
GROUP BY 1, 2
ORDER BY 1 DESC
LIMIT 14
