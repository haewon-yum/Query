...
, retention_by_app_bundle AS (
    ...
    (																								
        ...
        WHERE																								
        ((( TIMESTAMP(local_date) ) >= TIMESTAMP_ADD(TIMESTAMP('2024-05-13 00:00:00'), INTERVAL -24 hour)																								
        AND ( TIMESTAMP(local_date) ) <= TIMESTAMP_ADD(TIMESTAMP('2024-06-02 00:00:00'), INTERVAL 32 day)))																								
        AND 1=1 -- no filter on 'retention_by_app_bundle.selected_advertiser'																								
...
cv_table_install AS (																								
    ...
    *																								
    ...
    WHERE																								
    timestamp >= TIMESTAMP_ADD(TIMESTAMP_TRUNC(TIMESTAMP_TRUNC(TIMESTAMP('2024-05-13 00:00:00'), DAY, 'UTC'), MONTH), INTERVAL -24 hour)																								
    AND timestamp <= TIMESTAMP_ADD(TIMESTAMP(DATETIME_ADD(DATETIME(TIMESTAMP_TRUNC(TIMESTAMP_TRUNC(TIMESTAMP('2024-06-02 00:00:00'), DAY, 'UTC'), MONTH)), INTERVAL 1 MONTH)), INTERVAL 32 day)																								
    AND 1=1 -- no filter on 'retention_by_app_bundle.selected_advertiser'																								
...
--`focal-elf-631.prod_stream_view.cv`																								
WHERE																								
timestamp >= TIMESTAMP_ADD(TIMESTAMP('2024-05-13 00:00:00'), INTERVAL -24 hour)																								
AND timestamp <= TIMESTAMP_ADD(TIMESTAMP('2024-06-02 00:00:00'), INTERVAL 32 day)																								
AND 1=1 -- no filter on 'retention_by_app_bundle.selected_advertiser'																								
...
WHERE																								
_TABLE_SUFFIX BETWEEN FORMAT_TIMESTAMP('%Y%m%d',TIMESTAMP_ADD(TIMESTAMP_TRUNC(TIMESTAMP_TRUNC(TIMESTAMP('2024-05-13 00:00:00'), DAY, 'UTC'), MONTH), interval -24 hour))																								
AND FORMAT_TIMESTAMP('%Y%m%d', TIMESTAMP_ADD(TIMESTAMP(DATETIME_ADD(DATETIME(TIMESTAMP_TRUNC(TIMESTAMP_TRUNC(TIMESTAMP('2024-06-02 00:00:00'), DAY, 'UTC'), MONTH)), INTERVAL 1 MONTH)), interval 24 hour))																								
...
HAVING local_date BETWEEN DATE(TIMESTAMP_TRUNC(TIMESTAMP_TRUNC(TIMESTAMP('2024-05-13 00:00:00'), DAY, 'UTC'), MONTH))																								
AND DATE(TIMESTAMP(DATETIME_ADD(DATETIME(TIMESTAMP_TRUNC(TIMESTAMP_TRUNC(TIMESTAMP('2024-06-02 00:00:00'), DAY, 'UTC'), MONTH)), INTERVAL 1 MONTH)))																								
)																								
...
SELECT																								
r.os AS app_bundle_os,																								
r.app_bundle AS app_bundle,																								
da.dataai.publisher_name,																								
COALESCE(SUM(r.install), 0) AS total_install,																								
COALESCE(SUM(r.d1_retention), 0) AS d1_retention,																								
COALESCE(SUM(r.d1_retention), 0) / NULLIF(COALESCE(SUM(r.D0_install ), 0), 0) * 100 AS d1_retention_rate,																								
COALESCE(SUM(r.d3_retention), 0) AS d3_retention,																								
COALESCE(SUM(r.d3_retention), 0) / NULLIF(COALESCE(SUM(r.D0_install ), 0), 0) * 100 AS d3_retention_rate																								
FROM retention_by_app_bundle r LEFT JOIN `moloco-ae-view.athena.dim1_app` as da ON r.app_bundle = da.app_market_bundle																								
GROUP BY 1, 2, 3																								
ORDER BY																								
4 DESC																								
LIMIT 500																								
