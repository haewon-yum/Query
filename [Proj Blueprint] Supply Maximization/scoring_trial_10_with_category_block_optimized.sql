-- =====================================================================
-- Accessible Supply Scores at campaign level (optimized v2)
-- Portable version (CURRENT_DATE(), no @run_time dependency)
--
-- v2 optimizations over v1:
-- 1) Consolidate 3x fact_dsp_core scans into 1 base CTE
-- 2) Consolidate 2x bidrequest20* scans into 1 base CTE
-- 3) Scope apt_categories to only relevant app_bundles
-- 4) Fix LAT regex (missing {n} quantifiers)
-- 5) Remove unnecessary SELECT DISTINCT in summary join
-- 6) Deduplicate identical CASE expression in campaign_bids_without_target
-- =====================================================================
...
-- [Optimization 1] Consolidates 3 separate scans into 1 base CTE.
...
-- 8) Effective Targeting Split
-- Builds campaign profile with all targeting masks and has_effective_targeting boolean.
...
-- Splits campaigns into targeted vs non-targeted paths.
...
-- 10) Bidrequest Aggregation (Single Scan Strategy)
-- [Optimization 2] Unions country/os sets and scans bidrequest20* once.
-- bid_base feeds both detailed (targeted) and lightweight (non-targeted) aggregations.
bid_country_os_all AS (
  SELECT country, os FROM campaign_country_os_target
  UNION DISTINCT
  SELECT country, os FROM campaign_country_os_no_target
),

-- Single bidrequest scan with [Optimization 4] fixed LAT regex quantifiers.
bid_base AS (
  SELECT
    UPPER(b.country) AS country,
    UPPER(b.os) AS os,
    -- [Optimization 4] Fixed LAT regex: {8}, {4}, {3} quantifiers
    CASE
      WHEN IF(
        id_type IS NULL,
...
-- 10.1) Detailed bid dim for campaigns with targeting (by app_bundle, exchange, device_type).
bid_dim_targeted AS (
  SELECT
    country, os, is_lat, app_bundle, exchange, device_type,
    COUNT(*) AS total_bids
  FROM bid_base
  WHERE (country, os) IN (SELECT AS STRUCT country, os FROM campaign_country_os_target)
  GROUP BY country, os, is_lat, app_bundle, exchange, device_type
),

-- 10.2) Lightweight bid dim for campaigns without targeting (country, os, is_lat only).
bid_dim_light AS (
  SELECT
    country, os, is_lat,
    COUNT(*) AS total_bids
...
-- [Optimization 3] Scoped to only app_bundles that appeared in bidrequests.
apt_categories AS (
  SELECT
    app_bundle,
  ...
  WHERE app_categories IS NOT NULL
    ...
    AND app_bundle IN (SELECT DISTINCT app_bundle FROM bid_dim_targeted)
),

-- 12) Targeted Path Evaluation
-- Joins targeted campaigns to detailed bid dim and APT categories.
...
-- Computes targeting_pass and tracking_pass flags once per row.
...
-- 13) Bid Metrics by Campaign
-- 13.1) Targeted path: aggregates using pre-computed pass/fail flags.
...
-- 13.2) Non-targeted path: tgt_eligible = total, accessible = tracking_eligible.
-- [Optimization 6] Computes tracking_eligible once, reuses as accessible_bids.
...
-- [Optimization 5] Joins campaign_digest directly (no SELECT DISTINCT).
summary AS (
  SELECT
    s.campaign_id,
    cd.os,
...
-- 15) Blueprint Scores
-- 15.1) Target Accessible Supply Score
...
-- 15.2) Traffic Accessible Supply Score
...
-- 15.3) OS Coverage Score
-- Calculates advertiser-level OS mix quality by country vs market ratio.
-- [Optimization 1 continued] Reuses fact_base instead of rescanning fact_dsp_core.
...
-- 16) Final Merge and Overall Index
