...
flat_target AS (
  SELECT
    ...
    CASE
      ...
      WHEN key = 'blocked_categories'   THEN JSON_VALUE_ARRAY(t.condition_json, '$.blocked_categories')
      WHEN key = 'allowed_categories'   THEN JSON_VALUE_ARRAY(t.condition_json, '$.allowed_categories')
      ELSE []
    END AS vals
  FROM adgroup_raw ar
...
target_masks_raw AS (
  SELECT
    ...
    ARRAY_CONCAT_AGG(CASE WHEN key = 'blocked_categories'   THEN vals ELSE [] END) AS blocked_categories_all,
    ARRAY_CONCAT_AGG(CASE WHEN key = 'allowed_categories'   THEN vals ELSE [] END) AS allowed_categories_all
  FROM flat_target
...
target_masks AS (
  SELECT
    ...
    ARRAY(SELECT DISTINCT ctry FROM UNNEST(allowed_countries_all) AS ctry) AS allowed_countries,
    ARRAY(SELECT DISTINCT ctry FROM UNNEST(blocked_countries_all) AS ctry) AS blocked_countries,
    ...
    ARRAY(SELECT DISTINCT cat FROM UNNEST(blocked_categories_all) AS cat) AS blocked_categories,
    ARRAY(SELECT DISTINCT cat FROM UNNEST(allowed_categories_all) AS cat) AS allowed_categories
...
-- =====================================================================
-- AS-5.5. APT (App Profile Table) for category lookup
--         Used as canonical source for publisher app categories
-- =====================================================================
apt_categories AS (
  SELECT
    app_bundle,
    app_categories AS pub_categories  -- ARRAY<STRING> like ["IAB9", "IAB1"]
...
--       Categories are looked up separately via APT at filter time
-- =====================================================================
bid_dim AS (
  SELECT
  ...
  WHERE _TABLE_SUFFIX BETWEEN FORMAT_DATE('%y%m%d', spend_date_start)
                          AND FORMAT_DATE('%y%m%d', spend_date_end)
...
--       Category block is applied using APT lookup
-- =====================================================================
    ...
    SUM(
      CASE
        WHEN
          ...
          -- *** CATEGORY ALLOWLIST: if set, publisher must have at least one allowed category ***
          AND IF(tm.allowed_categories IS NULL OR ARRAY_LENGTH(tm.allowed_categories) = 0,
                 TRUE,  -- no allowed categories set → pass
                 -- Check that AT LEAST ONE of the publisher's categories is in allowed list
                 apt.pub_categories IS NOT NULL 
                 AND EXISTS (
                   SELECT 1 
                   FROM UNNEST(apt.pub_categories) AS pub_cat
                   WHERE pub_cat IN UNNEST(tm.allowed_categories)
                 )
          )
          -- *** CATEGORY BLOCKLIST: if ANY pub category matches ANY blocked category → filter out ***
          AND IF(tm.blocked_categories IS NULL OR ARRAY_LENGTH(tm.blocked_categories) = 0,
                 TRUE,  -- no blocked categories → pass
                 -- Check that NONE of the publisher's categories are in blocked list
                 -- If app has no categories in APT, it passes (conservative approach)
                 apt.pub_categories IS NULL 
                 OR NOT EXISTS (
                   SELECT 1 
                   FROM UNNEST(apt.pub_categories) AS pub_cat
                   WHERE pub_cat IN UNNEST(tm.blocked_categories)
                 )
          )
        THEN bd.total_bids
        ELSE 0
...
campaign_bids_without_target AS (
  SELECT
    ...
    -- accessible = tracking (no category blocks for campaigns without targeting)
    SUM(
      CASE
        WHEN cl.ad_tracking_allowance = 'NON_LAT_ONLY' THEN IF(bd.is_lat = 'No', bd.total_bids, 0)
        WHEN cl.ad_tracking_allowance = 'LAT_ONLY'     THEN IF(bd.is_lat = 'Yes', bd.total_bids, 0)
  ...
  CROSS JOIN UNNEST(cl.target_countries) AS tc
  JOIN bid_dim bd
    ON bd.country = UPPER(tc) AND bd.os = UPPER(cl.os)
  GROUP BY cl.campaign_id
),
...
-- =====================================================================
-- AS-10. Summary table (BidRequest-based only)
-- =====================================================================
summary AS (
  SELECT
    s.campaign_id,
  ...
  LEFT JOIN (SELECT DISTINCT campaign_id, os, target_countries, ad_tracking_allowance FROM campaign_digest) cd USING (campaign_id)
  ORDER BY s.total_bids DESC
),

-- =====================================================================
-- AS-11. target_accessible_supply_bidreq_score 
--        (includes category block impact)
-- =====================================================================
...
-- =====================================================================
-- AS-12. traffic_accessible_supply_bidreq_score
-- =====================================================================
...
-- =====================================================================
-- Merge Scores & Compute Overall Index
-- =====================================================================
merged AS (
...
overall_index AS (
  SELECT
    *,
    -- overall = (target_bids_score * traffic_bids_score) / 100
    ROUND(
      (
        MAX(
          CASE
