...
-- 1) Avoid regex/UNNEST explosion while extracting target JSON arrays.
-- 2) Split bidrequest aggregation into targeted (detailed dims) vs non-targeted (light dims).
-- 3) Compute targeting/category predicates once per joined row.
-- =====================================================================

WITH params AS (
  SELECT
    DATE_SUB(DATE(@run_time), INTERVAL 3 DAY) AS spend_date_start,
    DATE_SUB(DATE(@run_time), INTERVAL 1 DAY) AS spend_date_end
),
...
-- Build campaign-level targeting masks directly from JSON arrays.
target_masks_raw AS (
  SELECT
    ar.campaign_id,
...
-- Detailed bid dim only for campaigns that need targeting checks.
bid_dim_targeted AS (
  SELECT
...
-- Light bid dim for campaigns without targeting checks.
bid_dim_light AS (
  SELECT
    UPPER(b.country) AS country,
    UPPER(b.os) AS os,
