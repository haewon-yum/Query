...
-- =====================================================================
-- 5️⃣ LAT policy + target_countries from campaign_digest
-- =====================================================================
campaign_digest AS (
  SELECT
    c.campaign_id,
...
-- =====================================================================
-- ⭐ NEW: Bid-level table (campaign-view of market bids)
-- =====================================================================
bids AS (
  SELECT
    d.campaign_id,
...
-- =====================================================================
-- ⭐ Bid-level aggregation with tracking & targeting
-- =====================================================================
bid_agg AS (
  SELECT
    b.campaign_id,
    ...
    -- tracking_eligible_bids: LAT policy only
    COUNTIF(
      d.ad_tracking_allowance = 'DO_NOT_CARE'
      OR (d.ad_tracking_allowance = 'NON_LAT_ONLY' AND b.is_lat = 'No')
      OR (d.ad_tracking_allowance = 'LAT_ONLY'     AND b.is_lat = 'Yes')
    ) AS tracking_eligible_bids,

    -- tgt_eligible_bids: targeting only, ignore LAT
    COUNTIF(
      IF(tm.allowed_apps IS NULL OR ARRAY_LENGTH(tm.allowed_apps) = 0,
         TRUE,
         b.app_bundle IN UNNEST(tm.allowed_apps)
      )
      AND IF(tm.blocked_apps IS NULL,
             TRUE,
             b.app_bundle NOT IN UNNEST(tm.blocked_apps)
      )
      AND IF(tm.allowed_exchanges IS NULL OR ARRAY_LENGTH(tm.allowed_exchanges) = 0,
             TRUE,
             b.exchange IN UNNEST(tm.allowed_exchanges)
      )
      AND IF(tm.blocked_exchanges IS NULL,
             TRUE,
             b.exchange NOT IN UNNEST(tm.blocked_exchanges)
      )
      AND IF(tm.allowed_countries IS NULL OR ARRAY_LENGTH(tm.allowed_countries) = 0,
             TRUE,
             b.country IN UNNEST(tm.allowed_countries)
      )
      AND IF(tm.blocked_countries IS NULL,
             TRUE,
             b.country NOT IN UNNEST(tm.blocked_countries)
      )
      AND IF(tm.allowed_device_types IS NULL OR ARRAY_LENGTH(tm.allowed_device_types) = 0,
             TRUE,
             b.device_type IN UNNEST(tm.allowed_device_types)
      )
      AND IF(tm.blocked_device_types IS NULL,
             TRUE,
             b.device_type NOT IN UNNEST(tm.blocked_device_types)
      )
    ) AS tgt_eligible_bids,

    -- accessible_bids: tracking + targeting together
    COUNTIF(
      (
        d.ad_tracking_allowance = 'DO_NOT_CARE'
        OR (d.ad_tracking_allowance = 'NON_LAT_ONLY' AND b.is_lat = 'No')
  ...
  FROM bids b
  JOIN campaign_digest d USING (campaign_id)
...
-- =====================================================================
-- 🔟 Summary table in wide form (now with bid-level columns)
-- =====================================================================
summary AS (
  SELECT
    mo.campaign_id,
    ...
    -- ⭐ NEW: bid-level opportunity columns
    b.total_bids,
    b.tracking_eligible_bids,
    b.tgt_eligible_bids,
    b.accessible_bids,

    SAFE_DIVIDE(b.tracking_eligible_bids, b.total_bids) AS accessible_ratio_tracking_bids,
    SAFE_DIVIDE(b.tgt_eligible_bids,      b.total_bids) AS accessible_ratio_targeting_bids,
    SAFE_DIVIDE(b.accessible_bids,        b.total_bids) AS accessible_ratio_total_bids
...
-- =====================================================================
-- ⭐ NEW 1: target_accessible_supply_bids_score
-- =====================================================================
target_accessible_supply_bids_score AS (
  SELECT
    campaign_id,
    'target_accessible_supply_bids_score' AS blueprint_index,
    ROUND(accessible_ratio_targeting_bids * 100, 2) AS score,
        ARRAY_TO_STRING(
      ARRAY(
        SELECT x FROM UNNEST([
          IF(num_blocked_apps > 0,         'num_blocked_apps: '         || CAST(num_blocked_apps AS STRING),         NULL),
...
-- =====================================================================
-- ⭐ NEW 2: traffic_accessible_supply_bids_score
-- =====================================================================
traffic_accessible_supply_bids_score AS (
  SELECT
    campaign_id,
    'traffic_accessible_supply_bids_score' AS blueprint_index,
    ROUND(accessible_ratio_tracking_bids * 100, 2) AS score,
    IF(ad_tracking_allowance <> 'DO_NOT_CARE', ad_tracking_allowance, NULL) AS detail,
    CASE 
      WHEN os = 'ANDROID' AND ad_tracking_allowance='NON_LAT_ONLY'
        THEN 'Consider utlizing LAT traffic in case of CPI optimization.'
    ...
    *,
    ...
    ROUND(
      (
        MAX(
          CASE
            WHEN blueprint_index = 'target_accessible_supply_bids_score'
            THEN score
          END
        ) OVER (PARTITION BY campaign_id)
        *
        MAX(
          CASE
            WHEN blueprint_index = 'traffic_accessible_supply_bids_score'
            THEN score
          END
        ) OVER (PARTITION BY campaign_id)
      ) / 100.0,
      2
    ) AS overall_optimization_bids_score
  FROM merged
)
