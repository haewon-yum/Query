...
-- iOS primary: detect PA directly from attribution_method field
pa_primary AS (
  SELECT
    app_bundle AS mmp_bundle_id,
    SUM(IF(attribution_method IN ('probabilistic', 'fingerprint'), 1, 0)) > 0 AS pa_from_method,
    SUM(IF(attribution_method = 'UNKNOWN', 1, 0)) > 0 AS has_unknown_method
  FROM normalized_pb_data
  WHERE event_type = 'install'
...
-- iOS fallback: infer PA from MTID validity on LAT opt-out postbacks
-- Used when attribution_method is UNKNOWN (e.g. Tenjin doesn't send this field)
pa_fallback AS (
  SELECT
    tracking_bundle AS mmp_bundle_id,
    MAX(verdict.fp_status = 'ENABLED') AS pa_from_fp_status
  FROM `focal-elf-631.mmp_pb_summary.app_status`
  WHERE utc_date = (
    SELECT MAX(utc_date)
    FROM `focal-elf-631.mmp_pb_summary.app_status`
    WHERE utc_date >= DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY)
  )
  GROUP BY 1
),

-- iOS PA tab: primary signal first, fallback only when attribution_method was UNKNOWN
pa_tab AS (
  SELECT
    p.mmp_bundle_id,
    'IOS' AS os,
    CASE
      WHEN p.pa_from_method THEN TRUE
      WHEN p.has_unknown_method AND COALESCE(f.pa_from_fp_status, FALSE) THEN TRUE
      ELSE FALSE
    END AS pa_enabled
  FROM pa_primary p
  LEFT JOIN pa_fallback f ON p.mmp_bundle_id = f.mmp_bundle_id
),
